...
local function handlePedDamage(uAttacker, iWeap)
	if tblFires[source] then
		if iWeap == 42 and getElementData(localPlayer,"Unternehmen") == 3 then -- extinguisher
...
local function handlePedWaterCannon(uPed)
	...
	if tblFires[uPed] then
		if getElementModel(source) == 407 and getElementData(getVehicleController(source),"Unternehmen") == 3 then -- fire truck
		handleSmoke(uPed)
