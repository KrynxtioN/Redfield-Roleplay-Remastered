...
function destroyFireElement(uElement, uDestroyer)
	if tblFires[uElement] then
		...
		infobox(uDestroyer,loc(uDestroyer,"Unternehmen_66"):format(money),"info")
		setElementData(uDestroyer,"Geld",getElementData(uDestroyer,"Geld")+money)
		
		local counter = 0
		for _,v in pairs(getElementsByType("ped"))do
			if(getElementData(v,"FirePed") == true)then
				counter = counter + 1
			end
		end
		if(counter == 0)then
			for _,v in pairs(getElementsByType("player"))do
				if(getElementData(v,"loggedin") == 1)then
					if(getElementData(v,"Unternehmen") == 3)then
						outputChatBox(loc(v,"Unternehmen_67"),v,255,255,255,true)
					end
				end
			end
			RescueTeam.activeEinsatz = false
			RescueTeam.blipPos = {}
			RescueTeam.activeEinsatzText = nil
			addToLager(3,"Geld",1250,"unternehmenlager")
			
			for _,v in pairs(getElementsByType("player"))do
				triggerClientEvent(v,"RescueTeam.einsatzBlip",v)
			end
		end
...
function createFireElement(iX, iY, iZ, iSize, bDecaying)
	if tonumber(iX) and tonumber(iY) and tonumber(iZ) and tonumber(iSize) and iSize >= 1 and iSize <= 3 then
		...
		setElementData(uPed,"FirePed",true)
